"""Initial schema - all tables, RLS, indexes, and seed data
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("tenant_id", UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False),
        sa.Column("sector_code", sa.Text(), sa.ForeignKey("sectors.code"), nullable=True),
        sa.Column("company_name", sa.Text(), nullable=False),
        sa.Column("industry", sa.Text(), nullable=True),
        sa.Column("sub_industry", sa.Text(), nullable=True),
        sa.Column("state", sa.Text(), server_default="Tamil Nadu"),
        ...
        sa.Column("lead_score", sa.Integer(), server_default="50"),
        sa.Column("score_reason", sa.Text(), nullable=True),
        sa.Column("icp_match", sa.Text(), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False),
        sa.Column("name", sa.Text(), nullable=False),
        sa.Column("sector_codes", ARRAY(sa.Text()), nullable=True),
        sa.Column("status", sa.Text(), server_default="draft"),
        sa.Column("channel", sa.Text(), server_default="email"),
        sa.Column("segment_filter", JSONB(), server_default="{}"),
        sa.Column("daily_limit", sa.Integer(), server_default="100"),
        sa.Column("ai_tone", sa.Text(), server_default="professional"),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("tenant_id", UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False),
        sa.Column("campaign_id", UUID(as_uuid=True), sa.ForeignKey("campaigns.id", ondelete="CASCADE"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("step_id", UUID(as_uuid=True), sa.ForeignKey("campaign_steps.id"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False),
        sa.Column("lead_id", UUID(as_uuid=True), sa.ForeignKey("leads.id"), nullable=True),
        sa.Column("stage_id", UUID(as_uuid=True), sa.ForeignKey("pipeline_stages.id"), nullable=True),
        sa.Column("assigned_to", UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=True),
        sa.Column("title", sa.Text(), nullable=True),
        sa.Column("value_inr", sa.Integer(), nullable=True),
    ...
    # ------------------------------------------------------------------
    # INDEXES - Tenant ID (for all tenant-scoped tables)
    # ------------------------------------------------------------------
    op.create_index("idx_users_tenant", "users", ["tenant_id"])
    op.create_index("idx_leads_tenant", "leads", ["tenant_id"])
    op.create_index("idx_campaigns_tenant", "campaigns", ["tenant_id"])
    op.create_index("idx_campaign_steps_tenant", "campaign_steps", ["tenant_id"])
    op.create_index("idx_outreach_log_tenant", "outreach_log", ["tenant_id"])
    op.create_index("idx_ai_interactions_tenant", "ai_interactions", ["tenant_id"])
    op.create_index("idx_activities_tenant", "activities", ["tenant_id"])
    op.create_index("idx_deals_tenant", "deals", ["tenant_id"])
    op.create_index("idx_pipeline_stages_tenant", "pipeline_stages", ["tenant_id"])
    op.create_index("idx_api_keys_tenant", "api_keys", ["tenant_id"])

    # ------------------------------------------------------------------
    # INDEXES - Business logic
    # ------------------------------------------------------------------
    op.create_index("idx_leads_sector_code", "leads", ["sector_code"])
    op.create_index("idx_leads_stage", "leads", ["stage"])
    op.create_index("idx_leads_lead_score", "leads", ["lead_score"])
    op.create_index("idx_leads_email", "leads", ["email"])
    op.create_index("idx_campaigns_status", "campaigns", ["status"])
    op.create_index("idx_outreach_log_lead", "outreach_log", ["lead_id"])
    op.create_index("idx_outreach_log_status", "outreach_log", ["status"])
    op.create_index("idx_activities_lead", "activities", ["lead_id"])
    op.create_index("idx_deals_stage", "deals", ["stage_id"])
    ...
    # ------------------------------------------------------------------
    # SEED DATA - Sectors (9 sectors with full pain_points/value_props)
    # ------------------------------------------------------------------
    op.execute(
        """
    ...
    # ------------------------------------------------------------------
    # SEED DATA - Default Pipeline Stages for Demo Tenant
    # ------------------------------------------------------------------
    op.execute(
        """
    ...
    # ------------------------------------------------------------------
    # SEED DATA - Demo Admin User (password: admin123)
    # ------------------------------------------------------------------
    op.execute(
        """
    ...
    # ------------------------------------------------------------------
    # SEED DATA - Sample Leads (45 total, 5 per sector)
    # ------------------------------------------------------------------
...
def downgrade() -> None:
    ...
    # ------------------------------------------------------------------
    # Drop indexes (non-standard ones that won't auto-drop with tables)
    # ------------------------------------------------------------------
    op.execute("DROP INDEX IF EXISTS idx_leads_search")
